...
- `frontend-interaction-state.test.tsx` (9 tests — Prompt 5: Frontend Interaction & State-Management Bug Hunt Regression Suite):
  - Suite 1 (API URL Normalization & Sanitization): Defensively strips duplicated route segments (`/api/v1/api/v1/` normalized to `/api/v1/`) across all HTTP methods and fetch requests in `authenticatedFetch`.
  - Suite 2 (`apiClient` HTTP Verb Completeness): Tests complete verb coverage (`get`, `post`, `put`, `patch`, `delete`) with automatic JSON payload serialization, request header attachment, and `PATCH` method support.
  - Suite 3 (Form Double-Submission Mutexing & Rapid Clicking Safety): Validates `if (isSubmitting) return;` guard locks across all enterprise forms (`handleCreateInvoice`, `handleConfigureFurnace`, `handleClockInSubmit`, `handleCreateBatchOrder`, `handleConfirmTake`, `handleCompletePhysicalDispatch`), guaranteeing that rapid double-clicking or rapid Enter keystrokes only dispatch 1 single HTTP request to the backend.
  - Suite 4 (Zero Backend Error Masking): Verifies that 400, 403, 409, and 500 backend responses are never rendered as success notifications; proves that errors trigger red `AppAlert` components displaying the exact backend domain failure explanation.
  - Suite 5 (Modal & Dialog Dismissal Lifecycle): Verifies complete modal interaction lifecycle, asserting clean dismissal via Close button, keyboard `Escape` press, and modal backdrop click, without leaving unmounted or orphan modal artifacts.
  - Suite 6 (Navigation & Route Change State Reset): Verifies that active modals, drawers, and the global search Command Palette (`Ctrl+K`) automatically dismiss on route changes (`useLocation().pathname` listener in `MainLayout`), preventing stale modal state from surviving browser back/forward or direct route navigation.
  - Suite 7 (Concurrent 401 Single-Flight Token Refresh Mutex): Asserts that when multiple parallel requests receive 401 Unauthorized concurrently, the token refresh endpoint is invoked exactly once, queued requests are held in flight, and all requests seamlessly retry and resolve with the fresh access token.
