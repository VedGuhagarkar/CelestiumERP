...
export const createItemSchema: ValidationSchema = {
  body: z.object({
    ...
    conversionFactor: z.number().finite().positive('Conversion factor must be positive').max(1_000_000).optional(),
    minStockLevel: z.number().finite().min(0, 'Minimum stock level must be non-negative').max(1_000_000_000).default(0),
    reorderPoint: z.number().finite().min(0, 'Reorder point must be non-negative').max(1_000_000_000).default(0),
    maxStockLevel: z.number().finite().min(0).max(1_000_000_000).optional(),
    safetyStock: z.number().finite().min(0).max(1_000_000_000).optional(),
    currentStock: z.number().finite().min(0).max(1_000_000_000).default(0),
